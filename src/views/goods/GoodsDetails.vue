<template>
<div>
  <el-row>
    <el-col :span="12">
      <el-row>
        <el-col :span="12">
      <div>
        <p>华为荣耀play34+128g全网通 手机换过外屏，内屏原装 品牌:荣耀 型号:荣耀 Play 3</p>
      <img src="../../../static/images/banner01.jpg" style="width: 400px;height: 300px">
      </div>
    </el-col>
        <el-col :span="12">
      <el-row style="margin-top: 60px">
        <el-col :span="3"><p style="text-align: center;color: #808080">现价:</p></el-col>
        <el-col :span="3"><p style="color:#ff483c;font-size: 18px;margin-bottom: 0;margin-top: 16px">1499</p></el-col>
        <el-col :span="5"><p style="color:#808080;">元</p></el-col>
        <el-col :span="3"><p style="text-align: center;color: #808080">原价:</p></el-col>
        <el-col :span="3"><p style="color:#808080">14939</p></el-col>
        <el-col :span="1"><p style="color: #808080">元</p></el-col>
      </el-row>
      <el-row style="margin-bottom: 30px">
        <el-col :span="6">
          <span style="border:1px solid #ffa559;border-radius:3px;color:#ffa559 ">真实个人</span>
        </el-col>
        <el-col :span="6">
          <span style="border:1px solid #ffa559;border-radius:3px;color:#ffa559 ">担保交易</span>
        </el-col>
        <el-col :span="6">
          <span style="border:1px solid #ffa559;border-radius:3px;color:#ffa559 ">支持全国</span>
        </el-col>
        <el-col :span="6">
          <span style="border:1px solid #ffa559;border-radius:3px;color:#ffa559 ">在线沟通</span>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24"><el-button type="primary" >立即购买</el-button></el-col>
      </el-row>
    </el-col>
      </el-row>
      <!--  商家评论，有话说部分-->

      <el-row>
        <el-row>
          <div class="desc">
            <div class="desc-title">宝贝描述</div>
            <div class="desc-info">
              <img class="desc-info-pic" src="https://pic1.zhuanstatic.com/zhuanzh/fc6f7050-2830-435f-aace-b90cb548e5ca.jpg?w=100">
              <div class="desc-info-name">彤彤二手数码</div>
              <div class="desc-info-tips">有话说</div>
              <div class="desc-content">
                oppo Find x 全网通 【产品介绍】 名称：OPPO Findx 颜色：红色、蓝色 内存：8G+128G 尺寸：6.42寸 处理器：高通骁龙845 电池容量：3730mAh 【关于售后】 七天出现任何非人为的机器损坏包退换。 【关于本店】 本店只卖原装正品二手手机，不卖炸弹机、翻修机，所有机器支持线下任何门店验机，出现非人为问题包退换，关注店铺送五件套大礼包➕180天手机保修。
              </div>
            </div>
          </div>
        </el-row>
        <el-row>
          <div style="margin-top: 20px;font-size: 0">
            <div class="desc-title">宝贝图片</div>
            <img src="../../../static/images/banner01.jpg" style="width: 880px;">
            <img src="../../../static/images/banner02.jpg" style="width: 880px;">
            <img src="../../../static/images/banner03.jpg" style="width: 880px;">
          </div>
        </el-row>
        <el-row>
          <div class="desc-title">其他评论</div>
          <div class="desc-info">
            <img class="desc-info-pic" src="../../../static/images/banner03.jpg">
            <div class="desc-info-name">彤彤二手数码</div>
            <div class="desc-info-tips">有话说</div>
            <div class="desc-content">
              评论内容
            </div>
          </div>
        </el-row>
        <el-row style="margin-top: 30px">
          <el-button>我也要说</el-button>
        </el-row>
      </el-row>
    </el-col>
      <!--    用户信息介绍部分-->
    <el-col :span="6">
      <el-row>
      <div>
        <div class="user">
          <img class="user-img" src="https://pic1.zhuanstatic.com/zhuanzh/fc6f7050-2830-435f-aace-b90cb548e5ca.jpg?w=100">
          <div class="user-name">彤彤二手数码</div>
          <div class="user-detail">
            <div class="user-detail-title"><span>她</span>的交易</div>
            <div class="user-detail-tip">查看详细信息&gt;</div>
            <div class="user-detail-num">
             <el-row>
               <el-col>
                 <el-col :span="8" style="text-align: center;border-right: 1px solid #cecece;">
                   <el-row class="user-on-sale-goods">0</el-row>
                   <el-row class="user-on-sale-goods-introduce">在售宝贝</el-row>
                 </el-col>
                 <el-col :span="8" style="text-align: center;border-right: 1px solid #cecece;">
                   <el-row class="user-on-sale-goods">8</el-row>
                   <el-row class="user-on-sale-goods-introduce">累计交易</el-row>
                 </el-col>
                 <el-col :span="8" style="text-align: center;">
                   <el-row class="user-on-sale-goods">100%</el-row>
                   <el-row class="user-on-sale-goods-introduce">回复率</el-row>
                 </el-col>
               </el-col>
             </el-row>
            </div>
          </div>
        </div>
      </div>
      </el-row>
      <el-row>
        <div class="user">
          <div class="desc-title">宝贝描述</div>
          <el-row style="border-bottom: #e5e5e5 solid 1px">
            <el-col :span="12">
              <img src="../../../static/images/banner02.jpg" style="width: 140px;height: 105px">
            </el-col>
            <el-col :span="12">
              <el-row>
                【A91王者荣耀、和平精英、光遇、原神等各种大型游戏畅玩不卡 品牌:OPPO 型号:OPPO A91
              </el-row>
              <el-row>
                <span style="color:#ff483c; margin-left: 130px">680</span>元
              </el-row>
            </el-col>
          </el-row>
          <el-row style="border-bottom: #e5e5e5 solid 1px;padding: 15px 10px">
            <el-col :span="12">
              <img src="../../../static/images/banner01.jpg" style="width: 140px;height: 105px">
            </el-col>
            <el-col :span="12">
              <el-row>
                【A91王者荣耀、和平精英、光遇、原神等各种大型游戏畅玩不卡 品牌:OPPO 型号:OPPO A91
              </el-row>
              <el-row>
                <span style="color:#ff483c; margin-left: 130px">680</span>元
              </el-row>
            </el-col>
          </el-row>
          <el-row style="border-bottom: #e5e5e5 solid 1px;padding: 15px 10px">
            <el-col :span="12">
              <img src="../../../static/images/banner03.jpg" style="width: 140px;height: 105px">
            </el-col>
            <el-col :span="12">
              <el-row>
                【A91王者荣耀、和平精英、光遇、原神等各种大型游戏畅玩不卡 品牌:OPPO 型号:OPPO A91
              </el-row>
              <el-row>
                <span style="color:#ff483c; margin-left: 130px">680</span>元
              </el-row>
            </el-col>
          </el-row>

        </div>
      </el-row>
    </el-col>
  </el-row>
</div>
</template>

<script>
export default {
  name: "GoodsDetails"
}
</script>

<style scoped>
.desc-title{
  font-size: 16px;
  padding: 2px 10px;
  border-left: 2px solid #ff483c;
  margin-bottom: 14px;
  font-weight: bold;
}
.desc-info-pic{
  display: inline-block;
  width: 50px;
  height: 50px;
  border-radius: 50%;
  vertical-align: middle;
  margin-right: 10px;
}
.desc-info-name{
  display: inline-block;
  font-size: 14px;
  font-weight: bold;
  vertical-align: middle;
  margin-right: 10px;
}
.desc-info-tips{
  display: inline-block;
  font-size: 14px;
  color: #808080;
  vertical-align: middle;
}
.desc-content{
  font-size: 16px;
  padding: 34px 16px;
  background: #f8f8f8;
  border: 1px solid #eeeeee;
  border-radius: 3px;
  margin-top: 16px;
  position: relative;
  width: 50%;
}
.desc-content::after{
  content: '';
  position: absolute;
  width: 18px;
  height: 10px;
  background-image: url(data:image/png;base64,);
  overflow: hidden;
  background-position: -38px -547px;
  top: -9px;
  left: 15px;
}
.user{
  padding: 10px 10px 0 10px;
  margin-bottom: 20px;
  background-color: #f8f8f8;
}
.user-img{
  display: block;
  margin: 20px auto;
  border-radius: 50%;
  margin-bottom: 10px;
  width: 80px;
  height: 80px;
}
.user-name{
  text-align: center;
  font-size: 14px;
  margin-bottom: 20px;
}
.user-detail-title{
  padding: 0 10px;
  border-left: 3px solid #ff483c;

  font-weight: bold;
  float: left;
  vertical-align: middle;

}
.user-detail-tip{
  font-size: 12px;
  vertical-align: middle;
  float: right;
  margin-right: 24px;
  cursor: pointer;
}
/*.user-detail-num{*/
/*  clear: both;*/
/*  text-align: center;*/
/*  padding: 20px 0;*/
/*}*/
/*.user-detail-num-total {*/
/*  display: inline-block;*/
/*  border-right: 1px solid #cecece;*/
/*  padding-right: 14px;*/
/*}*/
/*.user-detail-num-total-num{*/
/*  font-size: 32px;*/
/*  text-align: center;*/
/*  color: gray;*/
/*}*/
/*.unit{*/
/*  display: none;*/
/*  margin: 0 2px;*/
/*  font-size: 14px;*/
/*}*/
.user-on-sale-goods{
  font-size: 32px;
  text-align: center;
  color: gray;
}
.user-on-sale-goods-introduce{
  font-size: 14px;
  text-align: center;
  color: gray;
}
</style>